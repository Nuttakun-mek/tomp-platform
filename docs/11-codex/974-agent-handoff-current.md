...
6. **iOS `UIBackgroundModes: ["location"]`** is missing from `infoPlist`. iOS
   stops delivering location the moment the app backgrounds — the entire point of
   background GPS. Fix before the first iOS build, not after.
