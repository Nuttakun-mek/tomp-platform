...
| **P0-4** atomic Timeline + command RPCs (migrations written + verified on a throwaway DB, not applied to prod) | `0025` (AFTER triggers → `timeline_events` in the business transaction for 8 events) and `0026` (`create_project_command`, `publish_project_command` RPCs). `npm run db:verify-schema` builds `0001→latest` clean and passes, including a forced-failure rollback test. **DB owner applies to staging then production** — [960](960-p0-4-transactional-timeline-execution.md) has the per-file app changes. `change apply` atomicity + idempotency keys still open. | `8d7ede0` + follow |
...
|---|---|
| **P0-4** remaining: `change apply` atomicity + idempotency | `0025` (triggers) + `0026` (project-create / publish RPCs) are written and verified on a throwaway DB but not yet applied — DB owner runs [960](960-p0-4-transactional-timeline-execution.md) staging→prod, then deploys the per-file app rewiring. Still unwritten: a `change_apply` command function (dynamic target mutation) and client-supplied idempotency keys. |
...
| **P1-3** authenticated E2E | The unauthenticated smoke is now honest (above). The *authenticated* flow — login → create project → QR → PIN → GPS → status → publish, asserting DB + Timeline side effects — still needs Playwright + a dedicated test user + isolated project prefix + CI wiring. 957 Batch H. |
