...
Base: through commit `410aaa2`, with production migration hygiene updated on 2026-09-10. **Migrations through `0030_driver_mobile_sessions_grants.sql` are applied to production and verified.**
...
> 2026-09-10 production DB status update: the old "run 3 commands" note is no
> longer pending. `0029` tracking was renamed, `0030_driver_mobile_sessions_grants.sql`
> was applied, and `--reconcile-checksums --yes` cleared the cosmetic checksum
> drift after a no-drift verification. `node scripts/apply-migrations.mjs --dry-run`
> now reports `Nothing to apply. Database is up to date.`
...
| **P1-2** checksum drift analysed and cleared | `scripts/check-migration-drift.mjs` (`npm run db:check-drift`) applies the repo history to a disposable Postgres and diffs the RBAC surface (35 RLS policies, 39 `role_permissions`, 6 helper fns) against production. **All match**: `0011/0018/0019/0020` edits were comments/idempotency only; later migrations reconciled the live objects. No forward-repair migration. `apply-migrations.mjs --reconcile-checksums --yes` was run on production on 2026-09-10 and cleared the `CHANGED` warning. Doc 961. | `55980f9` |
...
| **Batch G follow-up** `/api/driver/assignment` closed | Mobile packet read now requires the scoped driver session through cookie/header and ignores `?token=`. The mobile shell/client and shared `@tomp/api-client` contract were updated to call the session-based endpoint. | (this pass) |
...
| **P1-2** clean-DB migration test | New `scripts/verify-schema.mjs` (`npm run db:verify-schema`) stubs the Supabase `auth`/`storage`/`realtime` objects, applies every migration to a disposable Postgres, asserts required tables / RLS / constraints / role seeds. The checksum-drift files were later diffed against production and confirmed as metadata-only drift; no forward-repair migration was needed. | (this commit) |
...
|---|---|---|
| **Superseded production DB note** | The prior owner-action rows for checksum reconcile and applying `0028` are historical. They were completed on 2026-09-10 together with the `0029` rename and `0030` grant migration. | no action |
...
| **P1-2** reconcile the drift checksums on prod | Done on 2026-09-10. Analysis confirmed **no real schema drift** (doc 961), then checksum bookkeeping was reconciled. | no action |
| **0028/0029/0030** production migrations | Done on 2026-09-10. `0028` hot-read indexes, `0029` driver mobile sessions, and `0030` mobile-session service-role grants are applied; dry-run reports database up to date. | no action |
...
| **`/api/driver/assignment` session migration** | Done in this pass. The endpoint uses `resolveDriverSession(request)`, returns no raw token, and the mobile/shared clients use `x-driver-session`. | no action |
