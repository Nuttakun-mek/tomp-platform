...
| **P0-4** atomic Timeline (migration written, not applied) | `0025_timeline_write_triggers.sql` — `AFTER` triggers write `timeline_events` in the business write's transaction for the 8 standard events. **Two-step rollout, DB owner executes** — see [959](959-p0-4-transactional-timeline-execution.md): staging verify (incl. forced-rollback test) → remove redundant app calls → production. Multi-row all-or-nothing commands still need RPC functions (959 step 3). | `8d7ede0` |
