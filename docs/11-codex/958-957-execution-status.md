...
| **P0-4** remaining: multi-row atomic commands | The Timeline half is done (migration `0025` + note `959`). Still open: project create + members + owner-link, publish snapshot + status + lock, and change-apply must each be all-or-nothing across their tables — real RPC command functions + idempotency keys. 959 step 3. |
