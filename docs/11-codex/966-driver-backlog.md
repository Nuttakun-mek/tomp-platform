...
1. ~~`UIBackgroundModes` is not in `infoPlist`.~~ **Wrong — see the correction
   below.** It is generated by the `expo-location` plugin at prebuild because
   `isIosBackgroundLocationEnabled: true` is set. Nothing to do.
