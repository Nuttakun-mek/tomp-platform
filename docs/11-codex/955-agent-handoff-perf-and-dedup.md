...
**สถานะ:** ยังไม่เริ่ม · เขียนไว้ให้ agent อื่นทำต่อได้ทันที
...
**Step 8.2 — งานที่มอบหมาย (D8)**
`/assignments` (ทุกโครงการ) กับ `/projects/[id]/assignments` (ในโครงการ) ซ้อนกัน
- ให้ `/assignments` เป็น "งานของฉันวันนี้ ข้ามโครงการ" จริงๆ (กรองเฉพาะวันนี้ + งานที่ยังไม่จบ) หรือถ้าไม่มีคุณค่าต่างจากในโครงการ ให้ redirect ไป `/projects` แล้วลบออกจาก `NAV_SECTIONS`
- เลือกทางไหนก็ได้ แต่ต้องเหลือทางเดียว และ `lib/auth/nav-model.ts` ต้องสะท้อนของจริง
