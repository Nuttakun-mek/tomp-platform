        ...
        <header className="rounded-[28px] border border-white/70 bg-white/90 p-5 shadow-xl shadow-slate-300/30 backdrop-blur sm:p-6">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              <h1 className="mt-2 text-3xl font-bold tracking-tight sm:text-4xl">{tr("fleet.pageTitle")}</h1>
              <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">{tr("fleet.pageSubtitle")}</p>
            </div>
            <div className="flex items-center gap-2">
              <span className="inline-flex items-center gap-2 rounded-full border border-teal-200 bg-teal-50 px-3 py-2 text-xs font-bold text-teal-800">
                <Eye className="h-4 w-4" /> {tr("fleet.readOnly")}
              </span>
          ...
          <div className="mt-5 grid gap-3 sm:grid-cols-3">
            ...
            <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
              <p className="text-xs font-semibold text-slate-500">{tr("fleet.vehiclesInProject")}</p>
        ...
        <section className="grid gap-5 lg:grid-cols-[minmax(0,1.55fr)_minmax(360px,0.85fr)]">
          <div className="overflow-hidden rounded-[28px] border border-slate-200 bg-white shadow-xl shadow-slate-300/30">
            <div className="flex items-center justify-between border-b border-slate-100 px-5 py-4">
              <div>
                <p className="text-xs font-bold uppercase tracking-[0.18em] text-teal-700">{tr("fleet.liveMap")}</p>
            ...
            {points.length ? (
              ...
              <div className="grid h-[560px] place-items-center bg-slate-50 text-sm font-semibold text-slate-500">{tr("fleet.noPosition")}</div>
            )}
          </div>
          ...
          <aside className="space-y-4">
            ...
            <div className="rounded-[28px] border border-slate-200 bg-white p-4 shadow-xl shadow-slate-300/30">
              <div className="flex items-center justify-between">
                <h2 className="text-lg font-bold">{tr("fleet.vehiclesInProject")}</h2>
                ...
                {view.units.map((unit) => (
                  <article key={unit.callSignId} className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <p className="text-xs font-bold uppercase tracking-[0.14em] text-teal-700">{tr("fleet.callSign")}</p>
                    ...
                    <div className="mt-3 grid gap-2 text-sm text-slate-700">
                      <p className="flex items-center gap-2 font-semibold">
                        ...
                        {unit.vehicle ? `${unit.vehicle.plateNumber} · ${unit.vehicle.vehicleType}` : tr("fleet.noVehicle")}
                      </p>
