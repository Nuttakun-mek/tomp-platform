  ...
  return (
    <section className="rounded-md border border-slate-200 bg-white p-5 shadow-soft">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.16em] text-operation">Today Board</p>
          <h2 className="mt-1 text-lg font-semibold text-ink">กระดานปฏิบัติการวันนี้</h2>
        </div>
        <Link className="rounded-md border border-operation px-3 py-2 text-sm font-semibold text-operation" href="/projects">
          ดูโครงการ
        </Link>
      </div>
      <div className="mt-5 grid gap-4 lg:grid-cols-3">
        <div className="rounded-md border border-slate-200 bg-slate-50 p-4">
          ...
          <div className="mt-3 grid gap-2">
            {projects.slice(0, 4).map((project) => (
              <Link key={project.id} className="rounded-md bg-white px-3 py-2 text-sm font-semibold text-ink shadow-sm" href={`/projects/${project.id}`}>
        ...
        <div className="rounded-md border border-slate-200 bg-slate-50 p-4">
          <p className="text-sm font-semibold text-slate-500">Timeline ล่าสุด</p>
