...
export function OperationTimelinePanel({ events }: { events: TimelineEvent[] }) {
  return (
    <CommandPanel title="ลำดับเหตุการณ์ Timeline" eyebrow="Immutable Log">
      <div className="grid gap-4">
        {events.length ? (
          ...
          <p className="rounded-md border border-dashed border-slate-300 bg-slate-50 p-4 text-sm text-slate-600">ยังไม่มี Timeline สำหรับโครงการนี้</p>
        )}
      </div>
    </CommandPanel>
  );
