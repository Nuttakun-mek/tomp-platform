...
export function CommandCenterHeader({ project, liveCount, issueCount }: { project: Project; liveCount: number; issueCount: number }) {
  return (
    <section className="command-panel-dark overflow-hidden rounded-3xl text-white shadow-command">
      <div className="command-grid p-6">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="min-w-0">
            <p className="text-[11px] font-bold tracking-[0.2em] text-teal-200">MISSION CONTROL / ศูนย์ควบคุมปฏิบัติการ</p>
            <h1 className="mt-2 text-2xl font-semibold leading-tight lg:text-[32px]">{project.projectName}</h1>
            <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-300">
              {project.projectCode} · ติดตามงานที่จัดสรร GPS Timeline และรายการที่ต้องตัดสินใจ
            </p>
          </div>
          <div className="grid gap-2 text-sm">
            <span className="inline-flex items-center gap-2 rounded-full bg-emerald-400/15 px-3 py-1.5 font-semibold text-emerald-100">
              <StatusDot tone="success" pulse />
              คนขับส่ง GPS {liveCount} รายการ
            </span>
            <span className="inline-flex items-center gap-2 rounded-full bg-amber-400/15 px-3 py-1.5 font-semibold text-amber-100">
