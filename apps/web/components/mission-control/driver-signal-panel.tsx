...
export function DriverSignalPanel({ locations }: { locations: DriverLocation[] }) {
  return (
    <CommandPanel title="สัญญาณคนขับ" eyebrow="Driver Signal">
      <div className="grid gap-3">
        {locations.length ? (
          locations.slice(0, 6).map((location) => {
            ...
            return (
              <article key={location.id} className="flex items-center justify-between gap-3 rounded-md border border-slate-200 bg-slate-50 p-3">
                ...
                <span className="inline-flex items-center gap-2 text-xs font-semibold text-slate-600">
                  ...
                  {age < 60 ? `${age} วิ` : `${Math.round(age / 60)} นาที`}
                </span>
              </article>
            );
          })
        ) : (
          <p className="rounded-md border border-dashed border-slate-300 bg-slate-50 p-4 text-sm text-slate-600">ยังไม่มี GPS จากคนขับ</p>
        )}
      </div>
    </CommandPanel>
  );
