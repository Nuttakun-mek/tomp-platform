...
import { formatRelativeCompactTh } from "@/lib/format/relative-time-th";
      ...
      <div className="grid gap-3">
        {locations.length ? (
          locations.slice(0, 6).map((location) => {
            ...
            return (
              <article key={location.id} className="flex items-center justify-between gap-3 rounded-card border border-slate-200 bg-slate-50/80 p-3">
                ...
                <span className="inline-flex shrink-0 items-center gap-2 text-xs font-semibold text-slate-600">
                  ...
                  {formatRelativeCompactTh(location.recordedAt)}
                </span>
              </article>
            );
          })
