              ...
              return (
                <li key={task.id} className={`rounded-xl border p-3 ${completed ? "border-emerald-200 bg-emerald-50" : "border-slate-200"}`}>
                  <div className="flex items-center gap-3">
                    ...
                    {!completed && !item.deletedAt && item.operationalStatus !== "cancelled" ? <form action={action}><Button type="submit" variant="secondary" className="min-h-8 px-2.5 py-1 text-xs">ทำเสร็จแล้ว</Button></form> : null}
                  </div>
                </li>
              );
            })}
