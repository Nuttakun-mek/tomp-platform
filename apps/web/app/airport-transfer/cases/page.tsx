...
export default async function AirportTransferCasesPage({ searchParams }: CasesPageProps) {
  ...
  return (
    <>
      ...
      {params.trashed ? <div className="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-700">ย้ายเคสไปยัง “ข้อมูลที่ลบแล้ว” เรียบร้อย สามารถกู้คืนได้ภายหลัง</div> : null}
      <form className="grid items-stretch gap-3 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm md:grid-cols-[minmax(0,1fr)_180px_200px_88px]" method="get">
        <Input name="q" defaultValue={params.q} className="h-11 rounded-xl py-0" placeholder="ค้นหา Case ID, เที่ยวบิน หรือชื่อ" />
        <Select name="direction" defaultValue={params.direction || ""} className="h-11 rounded-xl py-0"><option value="">ทุกประเภท</option><option value="arrival">รับเข้า</option><option value="departure">ส่งออก</option></Select>
        <Select name="status" defaultValue={params.status || ""} className="h-11 rounded-xl py-0"><option value="">ทุกสถานะ</option><option value="needs_review">รอตรวจสอบ</option><option value="ready_to_assign">พร้อมจัดรถ</option><option value="assigned">จัดรถแล้ว</option><option value="completed">เสร็จสิ้น</option><option value="issue">มีปัญหา</option></Select>
