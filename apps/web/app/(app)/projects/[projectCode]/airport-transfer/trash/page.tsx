...
export default async function AirportTransferTrashPage({ params }: { params: Promise<{ projectCode: string }> }) {
        ...
        {cases.length ? cases.map((item) => (
          <article key={item.id} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
            <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
              ...
              <div className="flex items-start gap-2"><ButtonLink href={`/projects/${projectCode}/airport-transfer/cases/${item.id}`} variant="quiet">ดูประวัติ</ButtonLink><RestoreAirportTransferCaseButton caseId={item.id} /></div>
            </div>
          </article>
