...
export async function getMissionsByProjectIds(projectIds: readonly string[]): Promise<Mission[]> {
  ...
  const { client: supabase } = await resolveReadClient();
  if (!supabase) return demoOr(demoKernel.missions.filter((mission) => ids.includes(mission.projectId)), []);
  ...
  try {
    ...
    if (error || !data) return demoOr(demoKernel.missions.filter((mission) => ids.includes(mission.projectId)), []);
    return data.map(mapMission);
  } catch {
    return demoOr(demoKernel.missions.filter((mission) => ids.includes(mission.projectId)), []);
  }
}