...
export const getCallSignsByProjectId = cache(async function getCallSignsByProjectId(projectId: string): Promise<CallSign[]> {
  ...
  if (!supabase) return demoOr(demoKernel.callSigns.filter((callSign) => callSign.projectId === projectId), []);
  ...
  try {
    ...
    if (error || !data) return demoOr(demoKernel.callSigns.filter((callSign) => callSign.projectId === projectId), []);
    return data.map(mapCallSign);
  } catch {
    return demoOr(demoKernel.callSigns.filter((callSign) => callSign.projectId === projectId), []);
  }
});